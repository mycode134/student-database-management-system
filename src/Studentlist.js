...
function Studentlist() {
  ...
  const deleteData = (id) => {
    fetch("http://localhost:3001/Studentlist/" + id, { method: "DELETE" }).then(
      () => {
  ...
  const filterRecord=async (e)=>{
    e.preventDefault()
    return await axios.get(`http://localhost:3001/Studentlist?q=${value}`)
    .then((res)=>{
      setDate(res.data)
      setValue("")
    })
  ...
  const sortRecords=async (e)=>{
    ...
    setSort(value)
    return await axios.get(`http://localhost:3001/Studentlist?_sort=${value}&_order=asc`)
    .then((res)=>{
      setDate(res.data)
      setValue("")
    })
